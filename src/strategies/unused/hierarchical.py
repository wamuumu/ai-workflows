...
class HierarchicalStrategy(StrategyBase):
    ...
    def generate(self, context, max_retries, debug):
        ...
        while True:
            plan = planner_chat.send_message(next_message, category="generation", max_retries=max_retries)
        ...
        for subtask in subtasks:
            fragment = agents.generator.generate_structured_content(task_generation_prompt_with_tools, subtask, response_model, max_retries=max_retries)
        ...
        merged = agents.planner.generate_structured_content(merge_prompt, fragment_string, response_model, max_retries=max_retries)
