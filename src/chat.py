import json

from agents.cerebras import CerebrasAgent
from models.workflow import StructuredWorkflow
from workflows.utils import WorkflowUtils
from utils.prompts import SYSTEM_PROMPT, CHAT_SYSTEM_PROMPT, USER_PROMPTS
from tools.registry import ToolRegistry

# Initialize the agent
cerebras = CerebrasAgent()

# Set up the prompts
chat_prompt_with_tools = f"{CHAT_SYSTEM_PROMPT}\n{ToolRegistry.to_prompt_format()}\n"
user_prompt = USER_PROMPTS[0]

# Generate the workflow (chat-based with clarification)
chat_history = cerebras.chat(chat_prompt_with_tools, user_prompt, debug=True)
workflow = cerebras.generate_workflow_from_chat(chat_history, SYSTEM_PROMPT, response_model=StructuredWorkflow, debug=True)

print("\nGenerated Workflow:")
print(json.dumps(workflow.model_dump(), indent=2))

wf_utils = WorkflowUtils(tools=ToolRegistry.get_all())
wf_utils.generate_html(workflow)